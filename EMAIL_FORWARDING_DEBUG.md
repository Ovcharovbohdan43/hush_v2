...
### 4. Проверьте вызов Mailgun API

В логах теперь отображается вызов Mailgun HTTP API:
```
Mailgun endpoint resolved: ...
✓✓✓ Email forwarded successfully ...
```

Если Mailgun вернул ошибку, будет записан статус/тело ответа.

### 5. Проверьте переменные окружения

Убедитесь, что заданы:
- `SMTP_FROM` — адрес отправителя (верифицирован в Mailgun)
- `MAILGUN_API_KEY` — HTTP API ключ Mailgun
- `MAILGUN_DOMAIN` — ваш домен в Mailgun
- `MAILGUN_API_BASE_URL` — `https://api.mailgun.net` (или `https://api.eu.mailgun.net` для EU)
...
**Решение:** Проверьте настройки webhook в Mailgun и убедитесь, что URL доступен из интернета.
...
### Проблема 4: Блокировка по IP при проверке безопасности Mailgun
**Признак:** вебхук `/api/v1/incoming/mailgun` стабильно отвечает `401`, хотя подпись отключена. В логах указаны реальные IP (`35.210.111.12` и т.п.), отсутствующие в whitelist.

**Причина:** проверка `MAILGUN_IP_WHITELIST` в сервисе безопасности. Стандартные CIDR не покрывают все регионы Mailgun, поэтому запросы блокируются до обработки.

**Временное решение:** в переменной окружения `MAILGUN_IP_WHITELIST` указать `*` или `allow_all`, затем перезапустить сервис. Это разрешит все IP Mailgun и уберёт 401.

**Постоянное решение:** заменить `*` на актуальные диапазоны IP Mailgun для вашего региона, чтобы восстановить строгую проверку.
...
4. Если письмо не доходит до webhook — проверьте настройки Mailgun
5. Если письмо доходит, но не пересылается — проверьте логи на ошибки Mailgun API
